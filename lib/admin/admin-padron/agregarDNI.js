  ...
  agregarDni (e) {
    ...
    .then((res) => {
      this.setState({
        result: res,
        showSuccess: true,
        textSuccess: `¡Se agrego el DNI ${aux} al padrón!`
      }, this.resetForm )
    })
    .catch((err) => {
      this.setState({
        showWarning: true,
        textWarning: `No se a podido agregar a la persona con DNI ${aux} ¿Tal vez ya está en el padrón?`
      })
      console.log(err)
    })
  }
  ...
  render () {
    ...
    return (
      ...
      <div id="buscar-dni-component">
        ...
        <p>Agregue un nuevo DNI al padrón. Esto permite que una persona se pueda registrar en la plataforma.</p>
        <p>Importante:</p>
        <ul>
          <li>No se permiten duplicados en el padrón. Por favor evite errores.</li>
          <li>No es necesario agregarlos si realizan voto presencial con cuenta admin, al entrar el voto, se chequea si está en el padrón. De no estar, se lo agrega.</li>
        </ul>
