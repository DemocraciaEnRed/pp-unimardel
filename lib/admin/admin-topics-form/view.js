...
export default class TopicForm extends FormView {
  constructor (topic, forum, tags, facultades) {
    const locals = {
  ...
  switchOn () {
    ...
    const facultades = this.el[0].querySelectorAll('input[type="facultades"]')
    Array.prototype.forEach.call(facultades, tagsInput)

    facultadStore.findAll({field: 'nombre'}).then(facultades => {
      this.admitedFacultades = facultades.map(z => z.nombre)
    })
