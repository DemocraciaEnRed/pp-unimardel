...
app.get('/vote/byTopic/csv',
  ...
  function sendCsv (req, res, next) {
    ...
    var infoTopic = req.topics.map((t) => {
      ...
      if (t.facultad) {
        auxFacultad = req.facultades.find((z) => z._id.toJSON() == t.facultad)
      }
      return [
        t.id,
    ...
    var data = [['ID', 'Topic', 'Facultad', 'Cant. Votos']]
    ...
    json2csv(data, function (err, csv) {
      ...
      res.set({
        ...
        'Content-Disposition': `attachment; filename=votos-por-topic-ppmgp-${Math.floor((new Date()) / 1000)}.csv`
      })
      res.write(csv)
      res.end()
