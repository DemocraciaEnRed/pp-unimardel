...
exports.list = function list (opts) {
  ...
  return getPossibleOwners(opts).then(owners => {
    ...
    if (owners === null || owners.length > 0){
      ...
      if (opts.facultad) query.facultad = { $in: [opts.facultad] }
 ...
 * @param {Facultad} opts.facultad Facultad of the topic
...
exports.vote = function vote (opts) {
  ...
  return find()
    ...
    .populate(scopes.ordinary.populate)
    .exec()
    .then(doVote.bind(null, user, value, claustro, votacionAbierta))
    .then((topic) => scopes.ordinary.expose(topic, forum, user))
}
...
function doVote(user, value, topic, claustro, votacionAbierta) {
